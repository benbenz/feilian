...
\begin{algorithm*}[h]
  ...
  \label{alg:sanitizer}
  \KwIn{HTML tree $T$ with root node $r$}
  \KwOut{Sanitized HTML tree $T'$}
  ...
  \While{$R \neq \emptyset$}{
      ...
      \eIf{$\text{is\_invisible\_or\_no\_text}(v)$}{
          $\text{remove}(v)$
      }{
          $\text{remove\_attributes}(v)$
      }
  }
...
\begin{algorithm*}[h]
  ...
  \label{alg:condenser}
  \KwIn{
      $T$: HTML tree with root node $r$\;
      $\mathcal{T} = \{t_1, t_2, \dots, t_n\}$: Set of texts to preserve\;
      $d: \mathcal{S} \times \mathcal{S} \to \mathbb{R}^+$: Distance function\;
  }
  \KwOut{
      $T'$: Condensed HTML tree\;
  }

  $\mathcal{D} \gets \emptyset$ \tcp*{text $\to$ minimal distance}
  $\mathcal{E} \gets \emptyset$ \tcp*{text $\to$ list of XPaths}
  
  \ForEach{$(v, s) \in \text{TextNodes}(T)$}{
      $\delta \gets \infty$\;
      \ForEach{$t \in \mathcal{T}$}{
          $\delta' \gets d(s, t)$\;
          \If{$\delta' < \delta$}{
              $\delta \gets \delta'$\;
              $\mathcal{D}[s] \gets \delta$\;
              $\mathcal{E}[s] \gets \{\text{XPath}(v)\}$\;
          }
          \ElseIf{$\delta' = \delta$}{
              $\mathcal{E}[s] \gets \mathcal{E}[s] \cup \{\text{XPath}(v)\}$\;
          }
      }
  }

  $\mathcal{X} \gets \bigcup_{s \in \mathcal{T}} \mathcal{E}[s]$  \tcp{Set of XPaths to preserve}
  
  \ForEach{$(p, v) \in \text{NodesWithXPath}(T)$}{
      \If{$p \notin \mathcal{X}$}{
          $\text{RemoveChildren}(v)$\;
          $\text{ReplaceText}(v, \text{"..."})$\;
      }
  }
\end{algorithm*}
